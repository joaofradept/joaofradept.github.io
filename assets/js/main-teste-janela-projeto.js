...
$("#games .card").on("click", function () {
  ...
  showGameDetails(gameData, id, "#game-window", true);
});

$("#projects .card").on("click", function () {
  const id = $(this).data("id");
  showGameDetails(projectData, id, "#project-window", false);
});

function showGameDetails(gameData, id, windowElement, galleryExpanded) {
  const data = gameData[id];

  $(windowElement + " #project-title").text(data.title);
  $(windowElement + " #project-description").text(data.description);

  const $mediaContainer = $(windowElement + " #project-media-container");
  ...
  $(windowElement).addClass("visible");

  const tags = data.tags || [];
  const $tagContainer = $(windowElement + " #project-tags");
  $tagContainer.empty();
